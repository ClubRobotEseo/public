...
static volatile adversary_e next_adversary;
static volatile bool_e flag_new_adversary = FALSE;
...
{
	if(flag_new_adversary)
	{
		flag_new_adversary = FALSE;
		EYES_next_adversary(next_adversary);
	}
}

void EYES_set_adversary(adversary_e adversary)
{
	flag_new_adversary = TRUE;
	next_adversary = adversary;
}
...
void EYES_next_adversary(adversary_e next_adversary)
{
	adversary_e previous_adversary;
	TIMER1_stop();
	previous_adversary = (next_adversary == ADVERSARY_1)?ADVERSARY_2:ADVERSARY_1;
	...
	if(current_adversary != next_adversary){	//C'est toujours le cas, sauf peut être au début !
		...
		LEDS_inform_running_tsop(&(adversary_detections[previous_adversary]));
	}
	...
	if(current_adversary == ADVERSARY_1)
