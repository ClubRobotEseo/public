...
#define LEDS_TRACK_ADVERSARIES		LEDS_BLUE		//Couleur des leds utilisées pour indiquer la position de l'adversaire.
...
#define LED_LIFETIME					200000		//[us] //Durée de vie d'une led allumée pendant le selftest !
														//Choisir 100ms pour que ca clignote à chaque balise émetrice.
														//Choisir 200ms pour un affichage continu contre une balise.
...
static volatile Led_balise_color_e team_color = LEDS_TRACK_ADVERSARIES;

static volatile bool_e foe_detected = FALSE;
static volatile LEDS_adversary_t foe;
static volatile bool_e selftest_mode = FALSE;
static volatile Uint32 leds_for_selftest[NB_LEDS];
...
void LEDS_set_selftest_mode(bool_e new_selftest_mode)
{
	selftest_mode = new_selftest_mode;
	LEDS_reset_all_color();
}



void LEDS_inform_running_tsop(volatile EYES_adversary_detections_t * adversary_detection)
{
	Uint8 tsop,j;
	bool_e b;
	if(selftest_mode)	//On perd pas de temps si c'est pas nécessaire !
	{
		for(tsop=0; tsop<NB_LEDS; tsop++)
		{
			b = FALSE;
			for(j=0; j<=adversary_detection->index; j++)
			{
				if(adversary_detection->detections[j].counts[tsop] > 0)	//Il y a eu des choses vues par ce tsop
				{
					b = TRUE;
					break;
				}
			...
			if(b)
				leds_for_selftest[tsop] = LED_LIFETIME;	//On recharge la durée de vie de la led allumée si une détection est vue par ce TSOP.
		}
	}
}
...
void LEDS_mux_process_it(){
	...
	if(selftest_mode)
	{	//En mode selftest, on crache un spot de 3 leds RGB sur chaque TSOP qui a vu QCH !
		//On se contente ici de ne mettre à jour que la couleur courante.... c'est bien suffisant !
		Uint8 led;
		for(led=0;led<NB_LEDS;led++)
		{
			if(leds_for_selftest[led] >= PERIOD_IT_LEDS_MUX)
				leds_for_selftest[led] -= PERIOD_IT_LEDS_MUX;
			else
				leds_for_selftest[led] = 0;
			display_leds[index_color][led] = (leds_for_selftest[led])?NB_STEP_BY_LED:0;
		}
	}
...
void LEDS_manager_process_it(){
	...
	switch(state){
		...
		case WAIT_BEGIN :
			...
			LEDS_track_adversaries_ir(entrance, LEDS_TRACK_ADVERSARIES);
