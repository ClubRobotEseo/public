...
{
	PWM_run(0, PWM_TIM_MOTOR, PWM_CH_MOTOR_1);
	PWM_run(0, PWM_TIM_MOTOR, PWM_CH_MOTOR_2);
	last_duty_left = 0;
	last_duty_right = 0;
}
...
#if USE_NEW_POWER_DETECTION
	if(global.flags.powerAvailable){
		PWM_run( (Sint8)duty_right, PWM_TIM_MOTOR, PWM_CH_MOTOR_1);
		PWM_run( (Sint8)duty_left, PWM_TIM_MOTOR, PWM_CH_MOTOR_2);
	}else{
		PWM_stop(PWM_TIM_MOTOR, PWM_CH_MOTOR_1);
		PWM_stop(PWM_TIM_MOTOR, PWM_CH_MOTOR_2);
	}
#else
	PWM_run( (Sint8)duty_right, PWM_TIM_MOTOR, PWM_CH_MOTOR_2);
	PWM_run( (Sint8)duty_left, PWM_TIM_MOTOR, PWM_CH_MOTOR_1);
#endif
	/////////////////////////////////////////////////////////
}
