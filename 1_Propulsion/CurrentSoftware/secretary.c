...
void SECRETARY_process_main(void)
{
	...
	while(UART1_data_ready())
	{
		toggle_led(LED_UART);
			...
			if(u1rxToCANmsg(&receivedCanMsg_over_uart, UART1_get_next_msg(), NULL))
...
void SECRETARY_process_CANmsg(CAN_msg_t* msg, MAIL_from_to_e from)
{
	...
	switch (msg->sid)
	{
		...
		case PROP_SET_POSITION:
			...
			SECRETARY_process_send(STRAT_POS_UPDATED, WARNING_NO, 0, 0);
		break;

		case PROP_RUSH_IN_THE_WALL:
			SEQUENCES_rush_in_the_wall(msg->data.prop_rush_in_the_wall.teta, msg->data.prop_rush_in_the_wall.way, msg->data.prop_rush_in_the_wall.speed, ACKNOWLEDGE_ASKED, 0, 0, BORDER_MODE, (msg->data.prop_rush_in_the_wall.asser_rot)?CORRECTOR_ENABLE:CORRECTOR_TRANSLATION_ONLY, msg->data.prop_rush_in_the_wall.idTraj);
		break;
		...
		case DEBUG_PROPULSION_SET_COEF:
			if(msg->data.debug_propulsion_set_coef.id < PROPULSION_NUMBER_COEFS && msg->size >= SIZE_DEBUG_PROPULSION_SET_COEF)
			{
				if(msg->data.debug_propulsion_set_coef.id <= ODOMETRY_COEF_CENTRIFUGAL)
					ODOMETRY_set_coef(msg->data.debug_propulsion_set_coef.id, msg->data.debug_propulsion_set_coef.value, msg->data.debug_propulsion_set_coef.memorySet);
		...
		case DEBUG_PROPULSION_RESET_COEF:
			if(msg->data.debug_propulsion_reset_coef.id < PROPULSION_NUMBER_COEFS && msg->size >= SIZE_DEBUG_PROPULSION_SET_COEF)
			{
				if(msg->data.debug_propulsion_reset_coef.id <= ODOMETRY_COEF_CENTRIFUGAL)
					ODOMETRY_reset_coef(msg->data.debug_propulsion_reset_coef.id);
				else if(msg->data.debug_propulsion_reset_coef.id == GYRO_COEF_GAIN)
					GYRO_reset_coef(msg->data.debug_propulsion_reset_coef.id);
				else
					CORRECTOR_reset_coef(msg->data.debug_propulsion_reset_coef.id);
			}
			else
				debug_printf("WARNING : bad datas in DEBUG_PROPULSION_RESET_COEF message received\n");
			SECRETARY_send_coef(msg->data.debug_propulsion_reset_coef.id);	//feedback
		break;
		...
		case IHM_GET_SWITCH:
			...
			printf("Message de la simulation \n");
		...
		case PROP_ACTIVATE_SCAN_DISTRI:
			#if USE_SCAN_DISTRI
				SCAN_DISTRI_receivedCanMsg(msg);
			#endif
			break;
		case PROP_SCAN_DISTRI_HOKUYO:
			global.flags.distributor_hokuyo_scan = TRUE;
			break;
		default :
		break;
	}

	//si c'est un message de Debug, on le transmets à la brique QS_Simu qui est peut-être concernée.
	if(msg->sid & DEBUG_FILTER)
		SIMULATOR_processMain(msg, (from == FROM_UART));
...
void SECRETARY_send_selftest_result(bool_e result)
{
	...
	if(ADC_getValue(ADC_SENSOR_SMALL_LASER) < 15)
		msg.data.strat_prop_selftest_done.error_code[i++] = SELFTEST_PROP_LASER_SENSOR_RIGHT;

	if(global.flags.virtual_mode)	//L'odométrie est faite sur un robot virtuel parfait.
