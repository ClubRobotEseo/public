...
void ODOMETRY_init(){
	...
	bool_e odometryCoefTranslationReadSuccess;
	bool_e odometryCoefSymReadSuccess;
	bool_e odometryCoefRotationReadSuccess;
	bool_e odometryCoefCentrifugalReadSuccess;
	...
	odometryCoefTranslationReadSuccess = MEMORY_getCoefOdometry((Sint32 *)&coefs[ODOMETRY_COEF_TRANSLATION], ODOMETRY_COEF_TRANSLATION);
	odometryCoefSymReadSuccess = MEMORY_getCoefOdometry((Sint32 *)&coefs[ODOMETRY_COEF_SYM], ODOMETRY_COEF_SYM);
	odometryCoefRotationReadSuccess = MEMORY_getCoefOdometry((Sint32 *)&coefs[ODOMETRY_COEF_ROTATION], ODOMETRY_COEF_ROTATION);
	odometryCoefCentrifugalReadSuccess = MEMORY_getCoefOdometry((Sint32 *)&coefs[ODOMETRY_COEF_CENTRIFUGAL], ODOMETRY_COEF_CENTRIFUGAL);

	if(QS_WHO_AM_I_get()==SMALL_ROBOT)
	{
		//SMALL
		if(odometryCoefTranslationReadSuccess == FALSE)
			coefs[ODOMETRY_COEF_TRANSLATION] 	 = 	SMALL_ODOMETRY_COEF_TRANSLATION_DEFAULT;

		if(odometryCoefSymReadSuccess == FALSE)
			coefs[ODOMETRY_COEF_SYM] 		 	 = 	SMALL_ODOMETRY_COEF_SYM_DEFAULT;

		if(odometryCoefRotationReadSuccess == FALSE)
			coefs[ODOMETRY_COEF_ROTATION] 		 = 	SMALL_ODOMETRY_COEF_ROTATION_DEFAULT;

		if(odometryCoefCentrifugalReadSuccess == FALSE)
	...
	debug_printf("ODOMETRY_COEF_TRANSLATION : %lx (%s read)\n", coefs[ODOMETRY_COEF_TRANSLATION], (odometryCoefTranslationReadSuccess)?"Success":"Fail");
	debug_printf("ODOMETRY_COEF_SYM : %lx (%s read)\n", coefs[ODOMETRY_COEF_SYM], (odometryCoefSymReadSuccess)?"Success":"Fail");
	debug_printf("ODOMETRY_COEF_ROTATION : %lx (%s read)\n", coefs[ODOMETRY_COEF_ROTATION], (odometryCoefRotationReadSuccess)?"Success":"Fail");
	debug_printf("ODOMETRY_COEF_CENTRIFUGAL : %lx (%s read)\n", coefs[ODOMETRY_COEF_CENTRIFUGAL], (odometryCoefCentrifugalReadSuccess)?"Success":"Fail");

	if(!odometryCoefTranslationReadSuccess || !odometryCoefSymReadSuccess || !odometryCoefRotationReadSuccess || !odometryCoefCentrifugalReadSuccess){
		CAN_msg_t msg;
		msg.sid = STRAT_PROP_MEMORY_STARTUP_CHECK;
		msg.size = SIZE_STRAT_PROP_MEMORY_STARTUP_CHECK;
		msg.data.strat_prop_memory_startup.state = PROP_MEMORY_STARTUP_CHECK__FAIL_NOT_PRESENT;
		SECRETARY_send_canmsg(&msg);
	}else{
		CAN_msg_t msg;
		msg.sid = STRAT_PROP_MEMORY_STARTUP_CHECK;
		msg.size = SIZE_STRAT_PROP_MEMORY_STARTUP_CHECK;
		msg.data.strat_prop_memory_startup.state = PROP_MEMORY_STARTUP_CHECK__GOOD;
		SECRETARY_send_canmsg(&msg);
	}
...
void ODOMETRY_set_coef(PROPULSION_coef_e coef, Sint32 value, bool_e memorySet)
{
	...
	if(coef <= ODOMETRY_COEF_CENTRIFUGAL){
		...
		if(memorySet){
			CAN_msg_t msg;
			msg.sid = DEBUG_PROPULSION_SET_COEF_ANSWER;
			msg.size = SIZE_DEBUG_PROPULSION_SET_COEF_ANSWER;
			msg.data.debug_propulsion_set_coef_answer.state = MEMORY_setCoefOdometry(value, coef);
...
void ODOMETRY_set_init_pos()
{
	...
	KALMAN_init_pos(global.position.x, global.position.y, global.position.teta);
#endif
...
#warning cest pas homogène à gauche il y a /5ms pas à droite pour moi ce nest pas une vitesse (mm/5ms) mais une distance (mm) ... parcourue en 5 ms
	...
	deviation_x = -(coefs[ODOMETRY_COEF_CENTRIFUGAL]*(real_speed_rotation)*(real_speed_y))/128/128/1024/1024;	//[rad.65536/ms]*[mm/512/ms]
	deviation_y = (coefs[ODOMETRY_COEF_CENTRIFUGAL]*(real_speed_rotation)*(real_speed_x))/128/128/1024/1024;		//[rad.65536/ms]*[mm/512/ms]
...
#if USE_KALMAN_FILTER
Sint64 ODOMETRY_get_real_speed_translation(void) {
	return real_speed_translation;
}
#endif
