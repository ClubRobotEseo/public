...
void DEBUG_envoi_point_fictif_alteration_coordonnees_reelles(void)
{
	...
	else
	{
		...
		if(COPILOT_get_border_mode() != NOT_BORDER_MODE)
		{
			...
			Sint16 robotWidth = (I_AM_BIG()) ? BIG_ROBOT_WIDTH : SMALL_ROBOT_WIDTH;
			...
			// Spécificité terrain 2019

			// Pente au niveau des grands distributeurs
			if(global.position.x >= 1543 - robotSize &&
					((global.position.y > 450 - robotWidth/2 && global.position.y < 1050 + robotWidth/2) ||
					(global.position.y > 1950 - robotWidth/2 && global.position.y < 2550 + robotWidth/2)))
			{
				global.real_speed_translation = 0;
				global.position.x = 1543 - robotSize;
				we_touch_border = TRUE;
			}

			// Pente au niveau des balances
			if(global.position.x >= 1578 - robotSize &&
					((global.position.y >= 1050 + robotWidth/2 && global.position.y <= 1480 - robotWidth/2) ||
					(global.position.y >= 1520 + robotWidth/2 && global.position.y <= 1950 - robotWidth/2)))
			{
				global.real_speed_translation = 0;
				global.position.x = 1578 - robotSize;
				we_touch_border = TRUE;
			}

			// Tasseau de bois sépararant les balances
			if(global.position.x >= 1578 - robotSize && global.position.y > 1480 - robotWidth/2 && global.position.y < 1520 + robotWidth/2)
			{
				global.real_speed_translation = 0;
				global.position.x = 1578 - robotSize;
				we_touch_border = TRUE;
			...
			// Accélérateurs de particules (pas de prise en compte du stock du goldenium)
			if(global.position.x <= 35 + robotSize &&
					((global.position.y > 500 - robotWidth/2 && global.position.y < 1250 + robotWidth/2) ||
					(global.position.y > 1750 - robotWidth/2 && global.position.y < 1500 + robotWidth/2)))
			{
				global.real_speed_translation = 0;
				global.position.x = 35 + robotSize;
				we_touch_border = TRUE;
			}

			// Zone centrale des accélérateurs de particules
			if(global.position.x <= 30 + robotSize && global.position.y >= 1250 + robotWidth/2 && global.position.y <= 1750 - robotWidth/2)
			{
				global.real_speed_translation = 0;
				global.position.x = 30 + robotSize;
				we_touch_border = TRUE;
			}
