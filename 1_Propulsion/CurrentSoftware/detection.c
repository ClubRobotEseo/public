...
detection_zone_s zones_hokuyo[NB_DETECTION_ZONES]={
		// {{(GEOMETRY_point_t){0, 0}, (GEOMETRY_point_t){360, 0}, (GEOMETRY_point_t){360, 710},(GEOMETRY_point_t){0, 710}}, FALSE}, 	// Exemple
		{{(GEOMETRY_point_t){1378, 1500}, (GEOMETRY_point_t){1578, 1500}, (GEOMETRY_point_t){1578, 1050},(GEOMETRY_point_t){1378, 1050}}, FALSE}, 	// Zone devant la balance jaune
		{{(GEOMETRY_point_t){1378, 1500}, (GEOMETRY_point_t){1578, 1500}, (GEOMETRY_point_t){1578, 1950},(GEOMETRY_point_t){1378, 1950}}, FALSE}, 	// Zone devant la balance violette
		{{(GEOMETRY_point_t){1378, 1050}, (GEOMETRY_point_t){1578, 1050}, (GEOMETRY_point_t){1578, 450},(GEOMETRY_point_t){1378, 450}}, FALSE}, 	// Zone devant la pente jaune
		{{(GEOMETRY_point_t){1378, 1950}, (GEOMETRY_point_t){1578, 1950}, (GEOMETRY_point_t){1578, 2550},(GEOMETRY_point_t){1378, 2550}}, FALSE}, 	// Zone devant la pente violette
		{{(GEOMETRY_point_t){1578, 450}, (GEOMETRY_point_t){2000, 450}, (GEOMETRY_point_t){2000, 2550},(GEOMETRY_point_t){1578, 2550}}, FALSE}, 	// Zone des pentes (jaune et violette
	};

detection_zone_s zones_balise[NB_DETECTION_ZONES]={
	// {{(GEOMETRY_point_t){0, 0}, (GEOMETRY_point_t){360, 0}, (GEOMETRY_point_t){360, 710},(GEOMETRY_point_t){0, 710}}, FALSE}, 	// Exemple
		{{(GEOMETRY_point_t){1378, 1500}, (GEOMETRY_point_t){1578, 1500}, (GEOMETRY_point_t){1578, 1050},(GEOMETRY_point_t){1378, 1050}}, FALSE}, 	// Zone devant la balance jaune
		{{(GEOMETRY_point_t){1378, 1500}, (GEOMETRY_point_t){1578, 1500}, (GEOMETRY_point_t){1578, 1950},(GEOMETRY_point_t){1378, 1950}}, FALSE}, 	// Zone devant la balance violette
		{{(GEOMETRY_point_t){1378, 1050}, (GEOMETRY_point_t){1578, 1050}, (GEOMETRY_point_t){1578, 450},(GEOMETRY_point_t){1378, 450}}, FALSE}, 	// Zone devant la pente jaune
			...
			for(i = 0; i<BEACON_MAX_FOES; i++)
			{
				...
				if((msg->data.broadcast_beacon_adversary_position_ir.adv[i].error & ~TACHE_TROP_GRANDE) == AUCUNE_ERREUR){
					...
					if	(global.absolute_time - hokuyo_update_time > FOE_DATA_LIFETIME
							|| 	(		adversaries[HOKUYO_MAX_FOES+i].angle < -(PI4096 / 18)		//Angle entre 10° et 170° --> angle mort hokuyo + marge de 15° de chaque coté.
									&& 	adversaries[HOKUYO_MAX_FOES+i].angle > -(17 * PI4096 / 18) )
						)
					{
					...
					// Zones de transparence 2019

					// Exemple de transparence pour l'hokuyo
//					if(is_in_polygon(zones_hokuyo[DETECTION_START_ZONE_BLUE].polygon, NB_POINTS_BY_ZONE, pos, out_point, NULL)){
//						if(is_in_polygon(zones_hokuyo[DETECTION_NEXT_TO_START_ZONE_BLUE].polygon, NB_POINTS_BY_ZONE, adv_pos, out_point, NULL)){
//							adversaries[HOKUYO_MAX_FOES+i].enable = FALSE;
//							adversaries[HOKUYO_MAX_FOES+i].updated = FALSE;
//						}
//					}

					// Transparence lorsque le robot est devant la balance jaune
					if(is_in_polygon(zones_hokuyo[DETECTION_ZONE_FRONT_BALANCE_YELLOW].polygon, NB_POINTS_BY_ZONE, pos, out_point, NULL)){
						if(is_in_polygon(zones_hokuyo[DETECTION_ZONE_FRONT_BALANCE_PURPLE].polygon, NB_POINTS_BY_ZONE, adv_pos, out_point, NULL)){
							adversaries[HOKUYO_MAX_FOES+i].enable = FALSE;
							adversaries[HOKUYO_MAX_FOES+i].updated = FALSE;
						}
						if(is_in_polygon(zones_hokuyo[DETECTION_ZONE_SLOPES_AND_BALANCES].polygon, NB_POINTS_BY_ZONE, adv_pos, out_point, NULL)){
							adversaries[HOKUYO_MAX_FOES+i].enable = FALSE;
							adversaries[HOKUYO_MAX_FOES+i].updated = FALSE;
						}
					}

					// Transparence lorsque le robot est devant la balance violette
					if(is_in_polygon(zones_hokuyo[DETECTION_ZONE_FRONT_BALANCE_PURPLE].polygon, NB_POINTS_BY_ZONE, pos, out_point, NULL)){
						if(is_in_polygon(zones_hokuyo[DETECTION_ZONE_FRONT_BALANCE_YELLOW].polygon, NB_POINTS_BY_ZONE, adv_pos, out_point, NULL)){
							adversaries[HOKUYO_MAX_FOES+i].enable = FALSE;
							adversaries[HOKUYO_MAX_FOES+i].updated = FALSE;
						}
						if(is_in_polygon(zones_hokuyo[DETECTION_ZONE_SLOPES_AND_BALANCES].polygon, NB_POINTS_BY_ZONE, adv_pos, out_point, NULL)){
					...
					// Transparence lorsque le robot est devant la pente jaune
					if(is_in_polygon(zones_hokuyo[DETECTION_ZONE_FRONT_SLOPE_YELLOW].polygon, NB_POINTS_BY_ZONE, pos, out_point, NULL)){
						if(is_in_polygon(zones_hokuyo[DETECTION_ZONE_SLOPES_AND_BALANCES].polygon, NB_POINTS_BY_ZONE, adv_pos, out_point, NULL)){
							adversaries[HOKUYO_MAX_FOES+i].enable = FALSE;
							adversaries[HOKUYO_MAX_FOES+i].updated = FALSE;
						}
					...
					// Transparence lorsque le robot est devant la pente violette
					if(is_in_polygon(zones_hokuyo[DETECTION_ZONE_FRONT_SLOPE_PURPLE].polygon, NB_POINTS_BY_ZONE, pos, out_point, NULL)){
						if(is_in_polygon(zones_hokuyo[DETECTION_ZONE_SLOPES_AND_BALANCES].polygon, NB_POINTS_BY_ZONE, adv_pos, out_point, NULL)){
							adversaries[HOKUYO_MAX_FOES+i].enable = FALSE;
							adversaries[HOKUYO_MAX_FOES+i].updated = FALSE;
						}
	...
	for(i=0;i<HOKUYO_MAX_FOES + BEACON_MAX_FOES;i++)
	{
		if(adversaries[i].enable) //Pour tout les adversaires observés...
		{
			...
			// Exemple de transparence pour la balise
//			if(is_in_polygon(zones_balise[DETECTION_START_ZONE_BLUE].polygon, NB_POINTS_BY_ZONE, pos, out_point, NULL)){
//				zones_balise[DETECTION_NEXT_TO_START_ZONE_BLUE].enable = TRUE;
//			}

			// Transparence lorsque le robot est devant la balance jaune
			if(is_in_polygon(zones_hokuyo[DETECTION_ZONE_FRONT_BALANCE_YELLOW].polygon, NB_POINTS_BY_ZONE, pos, out_point, NULL)){
				zones_balise[DETECTION_ZONE_FRONT_BALANCE_PURPLE].enable = TRUE;
				zones_balise[DETECTION_ZONE_SLOPES_AND_BALANCES].enable = TRUE;
			}

			// Transparence lorsque le robot est devant la balance violette
			if(is_in_polygon(zones_hokuyo[DETECTION_ZONE_FRONT_BALANCE_PURPLE].polygon, NB_POINTS_BY_ZONE, pos, out_point, NULL)){
				zones_balise[DETECTION_ZONE_FRONT_BALANCE_YELLOW].enable = TRUE;
				zones_balise[DETECTION_ZONE_SLOPES_AND_BALANCES].enable = TRUE;
			}
