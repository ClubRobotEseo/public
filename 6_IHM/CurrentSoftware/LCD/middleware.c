	...
	for(i=0;i<LCD_NB_MAX_OBJECT;i++){
		if(objectTab[i].use && objectTab[i].toDestroy){
			switch(objectTab[i].type){
				...
				case TEXT:{
					Uint16 width, height;
					ILI9341_getStringSize((char *)objectTab[i].objectData.text.text, MIDDLEWARE_getFont(objectTab[i].objectData.text.fonts), &width, &height);
					ILI9341_drawFilledRectangle(objectTab[i].objectData.text.x,
												objectTab[i].objectData.text.y,
												objectTab[i].objectData.text.x + width,
												objectTab[i].objectData.text.y + height,
												background.color);
					}break;
...
void MIDDLEWARE_setTextColor(objectId_t id, objectColor_e colorText){
	if(!(id < LCD_NB_MAX_OBJECT))
		return;
	...
	switch(objectTab[id].type){
		case TEXT :{
			...
			objectTab[id].objectData.text.colorText = colorText;
			}break;
		...
		case BUTTON_BASE :{
				...
				objectTab[id].objectData.buttonBase.colorText = colorText;
			}break;
