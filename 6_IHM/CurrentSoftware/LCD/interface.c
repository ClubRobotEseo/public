...
void INTERFACE_processMain(void){
	...
	if(SWITCHS_get(SWITCH0_IHM) != lastStateSwitchLCD && actualIhm != INTERFACE_IHM_WAIT){
		LCD_OVER_UART_ihmControl(SWITCHS_get(SWITCH0_IHM));

		if(SWITCHS_get(SWITCH0_IHM))
			INTERFACE_setInterface(INTERFACE_IHM_POSITION);
		else
			INTERFACE_setInterface(INTERFACE_IHM_CUSTOM);

		lastStateSwitchLCD = SWITCHS_get(SWITCH0_IHM);
	}
