...
void ACT_arg_init(QUEUE_arg_t* arg, Uint16 sid, Uint8 cmd) {
	arg->msg.sid = sid;
	arg->msg.data.act_order.order = cmd;
...
void ACT_arg_init_with_param(QUEUE_arg_t* arg, Uint16 sid, Uint8 cmd, Uint16 param, bool_e run_now)  {
	arg->msg.sid = sid;
	arg->msg.data.act_order.order = cmd;
	arg->msg.data.act_order.act_optionnal_data[0] = param;
	arg->msg.data.act_order.run_now = run_now;
	arg->msg.size = SIZE_ACT_MSG;
	arg->fallbackMsg.sid = ACT_ARG_NOFALLBACK_SID;
	arg->timeout = ACT_ARG_DEFAULT_TIMEOUT_MS;
}
...
static void ACT_run_operation(queue_id_e act_id, bool_e init) {
	if(init) {
		...
		debug_printf("Sending operation, act_id: %d, sid: 0x%x, size: %d, order=%d,  data[0]:0x%x,  data[1]:0x%x\n", act_id, msg->sid , msg->size, msg->data.act_order.order, msg->data.act_order.act_optionnal_data[0], msg->data.act_order.act_optionnal_data[1]);
...
static void ACT_check_result(queue_id_e act_id) {
	...
	// Acknowledge des ordres actionneurs
...
void ACT_process_result(const CAN_msg_t* msg) {
	...
	if((msg->data.act_result.sid != (msgOrder->sid & 0xFF))
			|| (msg->data.act_result.cmd != msgOrder->data.act_order.order)){
		error_printf("Received result not match with actual order in queue : order(0x%x, %d) result(0x%x, %d)\n", msgOrder->sid & 0xFF, msgOrder->data.act_order.order, msg->data.act_result.sid, msg->data.act_result.cmd);
		return;
	}
	...
	switch(msg->data.act_result.result) {
		...
		default:	//ACT_RESULT_NOT_HANDLED et ACT_RESULT_FAILED (et les autres si ajouté)
			...
			switch(msg->data.act_result.error_code) {
				...
				case ACT_RESULT_ERROR_OVERHEATING:
					act_states[act_id].disabled = TRUE;
					act_states[act_id].recommendedBehavior = ACT_BEHAVIOR_DisableAct;
					error_printf("Error overheating ! act_id: 0x%x, cmd: 0x%x\n", msg->data.act_result.sid, msg->data.act_result.cmd);  //Erreur inconnu envoyé comme tel par la carte actionneur
					break;

				case ACT_RESULT_ERROR_OVERLOAD:
					act_states[act_id].disabled = TRUE;
					act_states[act_id].recommendedBehavior = ACT_BEHAVIOR_DisableAct;
					error_printf("Error overload ! act_id: 0x%x, cmd: 0x%x\n", msg->data.act_result.sid, msg->data.act_result.cmd);  //Erreur inconnu envoyé comme tel par la carte actionneur
					break;

				case ACT_RESULT_ERROR_OVERVOLTAGE_OR_UNDERVOLTAGE:
					act_states[act_id].disabled = TRUE;
					act_states[act_id].recommendedBehavior = ACT_BEHAVIOR_DisableAct;
					error_printf("Error overvoltage or undervoltage ! act_id: 0x%x, cmd: 0x%x\n", msg->data.act_result.sid, msg->data.act_result.cmd);  //Erreur inconnu envoyé comme tel par la carte actionneur
					break;

				case ACT_RESULT_ERROR_NOT_INITIALIZED:
					act_states[act_id].disabled = TRUE;
					act_states[act_id].recommendedBehavior = ACT_BEHAVIOR_DisableAct;
					error_printf("Error not initialized ! act_id: 0x%x, cmd: 0x%x\n", msg->data.act_result.sid, msg->data.act_result.cmd);  //Erreur inconnu envoyé comme tel par la carte actionneur
					break;
