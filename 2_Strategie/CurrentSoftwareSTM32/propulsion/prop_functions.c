...
error_e wait_move_and_wait_detection(PROP_moveAndDetection_data_s * data, trajectory_e trajectory_type, Uint8 nb_trajectory, Uint8 idLastTraj, avoidance_type_e avoidance_type, STRAT_endCondition_e end_condition, time32_t begin_time){
	...
	static error_e ret = IN_PROGRESS;
	static bool_e paused = FALSE;
	if(!paused)
		ret = IN_PROGRESS;	//si je ne suis pas en pause, on reset ret.
	if(SIMULATOR_is_pause_asked_by_toolkit())
	{
		global.flags.match_paused = TRUE;
		paused = TRUE;
	}
	else
	{
	...
	if(ret == IN_PROGRESS)	//si on a pas fini...
	{
	...
	//Le mode pause a pour effet de bloquer en attente le retour de la trajectoire demandée.
	//On TERMINE donc normalement la traj, sans rendre la main à la machine appelante !
	if(paused)	//on est en pause
		return IN_PROGRESS;
	else		//on est pas ou plus en pause
		return ret;
}
...
void PROP_set_coef(PROPULSION_coef_e coef, Sint32 value, bool_e memorySet){
	CAN_msg_t msg;
	...
	msg.data.debug_propulsion_set_coef.memorySet = memorySet;
	CAN_send(&msg);
}
