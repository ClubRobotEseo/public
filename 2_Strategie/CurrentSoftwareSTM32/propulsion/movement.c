...
Uint8 try_going_multipoint(displacement_t displacements[], Uint8 nb_displacements, Uint8 in_progress, Uint8 success_state, Uint8 fail_state, way_e way, avoidance_type_e avoidance, STRAT_endCondition_e end_condition)
{
	error_e sub_action;
	...
	switch(state){
		...
		case FAST_ROTATE:
			if(entrance){
				...
				if(way == ANY_WAY){	// Sens à décider
					if(absolute(GEOMETRY_modulo_angle(global.pos.angle - angle)) <= PI4096/2){	// Si l'angle à parcourir est entre -PI4096/2 et PI4096/2 on décide d'y aller en marche avant
						computedWay = FORWARD;
					}else{
						computedWay = BACKWARD;
					}
				}else{	// Sens explicite demandé
					computedWay = way;
				}


				if(angle > 0)
					angle -= ((computedWay == BACKWARD)? PI4096 : 0);
				else
					angle += ((computedWay == BACKWARD)? PI4096 : 0);
			}
		...
		case INIT_COEF:
			PROP_set_threshold_error_translation(75, FALSE);
			state = GO;
			break;

		case GO :
			sub_action = goto_pos_curve_with_avoidance((displacement_t[]){{{x, y},30}}, NULL, 1, computedWay, avoidance, END_AT_LAST_POINT, PROP_BORDER_MODE);
			switch(sub_action){
				case IN_PROGRESS:
					state = GO;
					break;
	...
	switch(state){
		case COMPUTE :
			...
			if(compute){
				...
				debug_printf("Point calculé dans try_advance : cos=%d ; sin=%d ; dist=%d ; [%d;%d]\n", cos, sin, dist, compute_point.x, compute_point.y);
				if(point != NULL){
 ...
 */
error_e goto_pos_curve_with_avoidance(displacement_t displacements[], displacement_curve_t displacements_curve[], Uint8 nb_displacements, way_e way, avoidance_type_e avoidance_type, STRAT_endCondition_e end_condition, prop_border_mode_e border_mode)
{
	CREATE_MAE(
			CHECK,
			LOAD_MOVE,
