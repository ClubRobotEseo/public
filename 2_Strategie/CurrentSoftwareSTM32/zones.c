...
static void init_a_zone(zone_id_e id, rectangle_t rectangle, zone_owner_e owner, zone_state_e state, Uint16 timeout)
{
	assert(id < NB_ZONES);
	zones[id].id = id;
...
//fonction appelée lors du lancement du match.
void ZONES_init(void)
{
	init_a_zone(ZONE_EXEMPLE,		(rectangle_t){0, COLOR_Y(0), 600, COLOR_Y(800)},			ZIS_BIG, ZS_Free, TIMEOUT_10s);
	debug_printf("Initialisation des zones effectuee.\n");
}



//fonction appelée en tâche de fond après le lancement du match.
void ZONES_process_main(void)
{
	int k;
...
void ZONE_CAN_process_msg(CAN_msg_t *msg) {
	...
	if(msg->data.xbee_zone_command.zone >= NB_ZONES) {
		info_printf("zone: unknown zone %d !!!\n", msg->data.xbee_zone_command.zone);
		return;
	}
...
char* stringNameZone(zone_id_e id)
{
	switch(id)
	{
		...
		default:
			return "UNKNOW_ZONE -> see the zones.c file to complete string";
	}
}
