...
void ENV_process_can_msg(CAN_msg_t * incoming_msg, bool_e bCAN, bool_e bU1, bool_e bU2, bool_e bXBee)
{
	...
	// Le ToolKit utilise l'uart 1 pour communiquer (le boolean est en état inverse)
...
void ENV_update(void){
	...
	while(UART1_data_ready())
	{
		...
		{
			if(parseInProgress == FALSE)	//si pas de message valide maintenant ET QUE pas de parse en cours, c'est qu'on reçoit du texte !
				char_from_user(c);
		}
...
/************************************* Récupération des envois de l'autre robot ***************************/
		case XBEE_START_MATCH:
			if(global.prop.calibrated)	//Vérification pour éviter de lancer un match si on est pas "sur le terrain"...et près à partir.
...
/************************************* Récupération des messages de la simulation ***************************/
		case IHM_GET_SWITCH:
...
void ENV_set_color(color_e color)
{
	/* changer la couleur */
	global.color = color;
	global.prop.calibrated = FALSE;	//si c'était le cas, le robot n'est plus calibré, on SET sa position !
