...
void BUZZER_play_fct_dont_use(time32_t duration_ms, BUZZER_note_e note, Uint8 nb_buzz, const char* fonction, Uint16 ligne)
{
	...
	{
		current_note = note;
		PWM_set_frequency(PWM_TIM_BUZZER, current_note);
	}
}
...
void BUZZER_process_main(void)
{
	...
	switch(state)
	{
		case INIT:
			...
			PWM_set_frequency(PWM_TIM_BUZZER, current_note);	//1kHz
			t = 0;
			nb_buzz_remaining = 0;
			state = IDLE;
			break;
		...
		case BUZZ:
			if(entrance)
			{
				PWM_run(50, PWM_TIM_BUZZER, PWM_CH_BUZZER);
				t = buzz_duration;
				nb_buzz_remaining--;
			}
			if(!t)
			{
				//Stop buzz
				PWM_stop(PWM_TIM_BUZZER, PWM_CH_BUZZER);
				if(nb_buzz_remaining)
					state = WAIT;
				else
					state = IDLE;
