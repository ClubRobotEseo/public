...
int term_cmd_clean(int argc, const char *argv[]) {
	...
	debug_printf("Un point = 10 fichiers de match traités, commence au match 500\n");

	for(match = 500; match>=0; match--)
	{
	...
	debug_printf("Vous pouvez maintenant reseter la carte Stratégie (si vous lancez un match maintenant, il ne sera pas enregistré). (la commande \"reset y\" peut faire l'affaire !\n");
