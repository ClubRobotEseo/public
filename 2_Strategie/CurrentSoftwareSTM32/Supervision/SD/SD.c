...
bool_e SD_open_file_for_next_match(void)
{
	...
	//match_id 			correspond à un nouveau numéro de match... si le précédent n'est pas "vide"

	if(read_match_id >= 500)
		match_id = 0;	//Le FAT16 limite à 512 fichiers par répertoire. On revient donc à 0 au delà 500. On assume l'écrasement des vieux fichiers... par flemme de complexifier le code
	else
...
*/
	...
	if(f_open(&file_read_match, path, FA_READ) == FR_OK)	//Ouverture en lecture seule.
	{
		...
		do
		{
			...
			// Ralentissement du débit de donnée car le simulateur n'encaisse pas
			volatile Uint32 x;
			for(x=0;x<10000;x++);
...
void SD_print_previous_match(void){
	debug_printf("L'index courant du match \"précédent\" est : %d\n",read_match_id);
