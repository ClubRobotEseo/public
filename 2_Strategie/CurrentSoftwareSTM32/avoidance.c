...
#if USE_FOE_ANALYSER
	if(global.flags.match_started && ELEMENTS_get_flag(F_ADV_START_ZONE_BLOC_TAKEN) == FALSE && foe_is_in_square(FALSE, 700, 1100, COLOR_Y(2150), COLOR_Y(2550), FOE_TYPE_HOKUYO)){
...
bool_e foe_is_in_square(bool_e verbose, Sint16 x1, Sint16 x2, Sint16 y1, Sint16 y2, foe_type_e foe_type)
{
	Uint8 i;
...
{
	return foe_is_in_square(verbose, x1, x2, COLOR_Y(y1), COLOR_Y(y2), foe_type);
}
