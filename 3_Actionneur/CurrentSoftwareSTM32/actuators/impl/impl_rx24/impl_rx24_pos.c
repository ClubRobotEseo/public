...
static void IMPL_RX24_POS_run_command_position(queue_id_t queueId, void* actuator_gen_ptr, bool_e init);
static void IMPL_RX24_POS_command_position_init(Actuator_servo_t* actuator);
static void IMPL_RX24_POS_command_position_run(Actuator_servo_t* actuator);
static void IMPL_RX24_POS_run_command_torque(queue_id_t queueId, void* actuator_gen_ptr, bool_e init);
static void IMPL_RX24_POS_add_torque_order(Actuator_servo_t *actuator, Uint8 cmd_index);
...
bool_e IMPL_RX24_POS_CAN_process_msg(void *actuator_gen_ptr, CAN_msg_t* msg) {
	...
	if(msg->sid == actuator->sid){
		...
		if(order_is_valid) {
			...
			ACTQ_push_operation_from_msg(msg, actuator->queue_id, &IMPL_RX24_POS_run_command_position, param, TRUE);

			// Ajoute l'ordre d'asservissement en couple si necessaire
			if(kind == CMD_ASSER_TORQUE) {
				IMPL_RX24_POS_add_torque_order(actuator, param);
			}
		} else {
...
void IMPL_RX24_POS_get_config(void *actuator_gen_ptr, CAN_msg_t *incoming_msg) {
	...
	switch(incoming_msg->data.act_set_config.config){
		case POSITION_CONFIG:
			IMPL_RX24_POS_get_position_config(actuator, &order, &pos);
...
static void IMPL_RX24_POS_run_command_position(queue_id_t queueId, void* actuator_gen_ptr, bool_e init) {
	...
	if(queueId == actuator->queue_id) {
		if(init) {
			IMPL_RX24_POS_command_position_init(actuator);
		} else {
			IMPL_RX24_POS_command_position_run(actuator);
		}
	}
}
...
static void IMPL_RX24_POS_command_position_run(Actuator_servo_t* actuator) {
	...
	if(global.flags.virtual_mode) {
		...
		if(ACTQ_check_status_rx24(actuator->queue_id, actuator->servo.id, wanted_goal, current_goal, actuator->servo.epsilon, actuator->servo.timeout, actuator->servo.large_epsilon, &result, &error_code, &line)) {
	...
	if(actuator->warner.activated)
		IMPL_RX24_POS_check_warner(actuator, current_goal);
}
...
static void IMPL_RX24_POS_add_torque_order(Actuator_servo_t *actuator, Uint8 cmd_index) {
	...
	if(threshold <= 100) {
		QUEUE_add(actuator->queue_id, &IMPL_RX24_POS_run_command_torque, (QUEUE_arg_t){ACT_SERVO_ASSER_TORQUE, cmd_index, &ACTQ_finish_SendNothing, TRUE});
	} else {
...
void IMPL_RX24_POS_set_warner(void* actuator_gen_ptr, CAN_msg_t *msg) {
	...
	debug_printf("Warner recieved\n");

	if(warner_is_valid){
		debug_printf("Warner valid\n");
		actuator->warner.warner_value = warner_pos;
		actuator->warner.activated = TRUE;
		actuator->warner.last_value = RX24_get_position(actuator->servo.id);
	} else {
...
void IMPL_RX24_POS_process_it(void* actuator_gen_ptr) {
	...
	if(actuator->servo.torque_asser.activated) {
		if(actuator->servo.torque_asser.ask_to_finish) {
			...
			Sint8 wanted_load = 0;
			Sint8 current_load = - RX24_get_load_percentage(actuator->servo.id);
			Uint16 current_pos = RX24_get_position(actuator->servo.id);
			...
			if(!RX24_set_position(actuator->servo.id, new_pos)) {
