...
#include "impl_ax12_pos.h"  // utile pour les warners en position
...
static void IMPL_AX12_SPEED_command_init(Actuator_servo_t* actuator);
static void IMPL_AX12_SPEED_command_run(Actuator_servo_t* actuator);
//static void IMPL_AX12_SPEED_check_warner(Actuator_servo_t* actuator, Uint16 pos);
 ...
 */
bool_e IMPL_AX12_SPEED_CAN_process_msg(void *actuator_gen_ptr, CAN_msg_t* msg) {
	Actuator_servo_t* actuator = (Actuator_servo_t*) actuator_gen_ptr;
	if(msg->sid == actuator->sid){
		...
		if(order_is_valid) {
			ACTQ_push_operation_from_msg(msg, actuator->queue_id, &IMPL_AX12_SPEED_run_command, param, TRUE);
		} else {
 ...
 * @param msg le message CAN reçu.
 */
void IMPL_AX12_SPEED_set_config(void *actuator_gen_ptr, CAN_msg_t* msg) {
	Actuator_servo_t* actuator = (Actuator_servo_t*) actuator_gen_ptr;
 ...
 */
void IMPL_AX12_SPEED_get_config(void *actuator_gen_ptr, CAN_msg_t *incoming_msg) {
	Actuator_servo_t* actuator = (Actuator_servo_t*) actuator_gen_ptr;
	bool_e error = FALSE;
	CAN_msg_t msg;
	...
	switch(incoming_msg->data.act_set_config.config){
		case POSITION_CONFIG:
			...
			if(global.flags.virtual_mode) {
				...
				msg.data.act_get_config_answer.act_get_config_data.act_get_config_pos_answer.pos = AX12_get_position(actuator->servo.id);
			}
			break;
		case TORQUE_CONFIG:
			if(global.flags.virtual_mode) {
 ...
 */
void IMPL_AX12_SPEED_run_command(queue_id_t queueId, void* actuator_gen_ptr, bool_e init) {
	Actuator_servo_t* actuator = (Actuator_servo_t*) actuator_gen_ptr;
	...
	if(queueId == actuator->queue_id) {
		if(init)
			IMPL_AX12_SPEED_command_init(actuator);
		else
			IMPL_AX12_SPEED_command_run(actuator);
	}
}
...
static void IMPL_AX12_SPEED_command_init(Actuator_servo_t* actuator) {
	...
	if(order == ACT_DEFAULT_STOP) {
		...
		ax12_goal_speed = QUEUE_get_arg(actuator->queue_id)->param;
	}
	...
	if(ax12_goal_speed == ERROR_ACT_VALUE) {
	...
	if(global.flags.virtual_mode) {
		...
		if(!AX12_set_speed_percentage(actuator->servo.id, ax12_goal_speed)) {	//Si la commande n'a pas été envoyée correctement et/ou que l'AX12 ne répond pas a cet envoi, on l'indique à la carte stratégie
...
static void IMPL_AX12_SPEED_command_run(Actuator_servo_t* actuator) {
	...
	if(global.flags.virtual_mode) {
		...
		if(ACTQ_check_status_ax12(actuator->queue_id, actuator->servo.id, QUEUE_get_arg(actuator->queue_id)->param, speed, actuator->servo.epsilon, actuator->servo.timeout, actuator->servo.large_epsilon, &result, &error_code, &line)) {
	...
	if(actuator->warner.activated){
		pos = AX12_get_position(actuator->servo.id);
		IMPL_AX12_POS_check_warner(actuator, pos);
	}
}
 ...
 * @param msg le message CAN reçu.
 */
void IMPL_AX12_SPEED_set_warner(void* actuator_gen_ptr, CAN_msg_t *msg) {
	IMPL_AX12_POS_set_warner(actuator_gen_ptr, msg);
}
