...
#define LOG_PREFIX "[AX12_POS_D]  "
#define LOG_COMPONENT OUTPUT_LOG_COMPONENT_IMPL_AX12_POS_DOUBLE
#include "../QS/QS_outputlog.h"

// Les fonctions internes au fonctionnement de l'actionneur
static void IMPL_AX12_POS_DOUBLE_init_servo(Actuator_servo_double_t* actuator);
static void IMPL_AX12_POS_DOUBLE_get_position_config(Actuator_servo_double_t* actuator, ACT_order_e *order_ptr, Uint16 *pos_ptr);
static void IMPL_AX12_POS_DOUBLE_run_command_position(queue_id_t queueId, void* actuator_gen_ptr, bool_e init);
static void IMPL_AX12_POS_DOUBLE_command_position_init(Actuator_servo_double_t* actuator);
static void IMPL_AX12_POS_DOUBLE_command_position_run(Actuator_servo_double_t* actuator);
static void IMPL_AX12_POS_DOUBLE_add_torque_order(Actuator_servo_double_t *actuator, Uint8 cmd_index);
static void IMPL_AX12_POS_DOUBLE_run_command_torque(queue_id_t queueId, void* actuator_gen_ptr, bool_e init);
static void IMPL_AX12_POS_DOUBLE_check_warner(Actuator_servo_double_t* actuator, Uint16 pos);
...
void IMPL_AX12_POS_DOUBLE_init(void* actuator_gen_ptr) {
	...
	AX12_init();
	//IMPL_AX12_POS_DOUBLE_init_servo(actuator); // Inutile
}
...
void IMPL_AX12_POS_DOUBLE_reset_config(void* actuator_gen_ptr) {
	...
	IMPL_AX12_POS_DOUBLE_init_servo(actuator);
}
...
static void IMPL_AX12_POS_DOUBLE_init_servo(Actuator_servo_double_t* actuator) {
	...
	bool_e is_ready[2];
	for(i = 0; i < 2; i++) {
		is_ready[i] = AX12_is_ready(actuator->servo[i].id);
	}
	...
	if(actuator->is_initialized == FALSE && is_ready[0] && is_ready[1]) {
		...
		for(i = 0; i < 2; i++) {
			AX12_config_set_lowest_voltage(actuator->servo[i].id, AX12_MIN_VOLTAGE);
			AX12_config_set_highest_voltage(actuator->servo[i].id, AX12_MAX_VOLTAGE);
			AX12_set_torque_limit(actuator->servo[i].id, actuator->servo[i].max_torque);
			AX12_config_set_temperature_limit(actuator->servo[i].id, actuator->servo[i].max_temperature);

			AX12_config_set_maximal_angle(actuator->servo[i].id, actuator->servo[i].max_angle);
			AX12_config_set_minimal_angle(actuator->servo[i].id, actuator->servo[i].min_angle);
			AX12_set_move_to_position_speed(actuator->servo[i].id, actuator->servo[i].speed);

			AX12_config_set_error_before_led(actuator->servo[i].id, AX12_BEFORE_LED);
			AX12_config_set_error_before_shutdown(actuator->servo[i].id, AX12_BEFORE_SHUTDOWN);

			debug_printf("AX12 n°%d init config DONE\n", actuator->servo[i].id);
		}
	}else if(actuator->is_initialized == FALSE){
		for(i = 0; i < 2; i++) {
			if(is_ready[i]) {
				debug_printf("AX12 n°%d init config FAIL : ready\n", actuator->servo[i].id);
			} else {
				debug_printf("AX12 n°%d init config FAIL : not ready\n", actuator->servo[i].id);
			}
		}
	}
}
...
error_e IMPL_AX12_POS_DOUBLE_init_pos(void *actuator_gen_ptr) {
	...
	for(i = 0; i < 2; i++) {
		if(!AX12_set_position(actuator->servo[i].id, actuator->servo[i].init_pos))
			debug_printf("AX12 n°%d is not here\n", actuator->servo[i].id);
		else
			debug_printf("AX12 n°%d has been initialized in position\n", actuator->servo[i].id);
	}
...
void IMPL_AX12_POS_DOUBLE_stop(void *actuator_gen_ptr) {
	...
	Uint8 i;
	for(i = 0; i < 2; i++) {
		AX12_set_torque_enabled(actuator->servo[i].id, FALSE);
	}
}
...
bool_e IMPL_AX12_POS_DOUBLE_CAN_process_msg(void *actuator_gen_ptr, CAN_msg_t* msg) {
	...
	if(msg->sid == actuator->sid){
		...
		if(order_is_valid) {
			...
			ACTQ_push_operation_from_msg(msg, actuator->queue_id, &IMPL_AX12_POS_DOUBLE_run_command_position, param, TRUE);

			// Ajoute l'ordre d'asservissement en couple si necessaire
			if(kind[0] == CMD_ASSER_TORQUE && kind[1] == CMD_ASSER_TORQUE) {
				IMPL_AX12_POS_DOUBLE_add_torque_order(actuator, param);
...
void IMPL_AX12_POS_DOUBLE_set_config(void *actuator_gen_ptr, CAN_msg_t* msg) {
	...
	Uint8 i;
	for(i = 0; i < 2; i++) {
		ACTMGR_config_AX12(&(actuator->servo[i]), msg);
	}
}
...
void IMPL_AX12_POS_DOUBLE_get_config(void *actuator_gen_ptr, CAN_msg_t *incoming_msg) {
	...
	switch(incoming_msg->data.act_set_config.config){
		case POSITION_CONFIG:
			IMPL_AX12_POS_DOUBLE_get_position_config(actuator, &order, &pos);
		...
		case TORQUE_CONFIG:
			if(global.flags.virtual_mode) {
				...
				msg.data.act_get_config_answer.act_get_config_data.torque = AX12_get_speed_percentage(actuator->servo[0].id);
			}
			break;
		case TEMPERATURE_CONFIG:
			if(global.flags.virtual_mode) {
				...
				msg.data.act_get_config_answer.act_get_config_data.temperature = AX12_get_temperature(actuator->servo[0].id);
			}
			break;
		case LOAD_CONFIG:
			if(global.flags.virtual_mode) {
				...
				msg.data.act_get_config_answer.act_get_config_data.load = AX12_get_load_percentage(actuator->servo[0].id);
			}
			break;
		default:
			error = TRUE;
...
static void IMPL_AX12_POS_DOUBLE_get_position_config(Actuator_servo_double_t* actuator, ACT_order_e *order_ptr, Uint16 *pos_ptr) {
	...
	if(global.flags.virtual_mode) {
		...
		pos = (Sint16) AX12_get_position(actuator->servo[0].id);
	}
...
static void IMPL_AX12_POS_DOUBLE_run_command_position(queue_id_t queueId, void* actuator_gen_ptr, bool_e init) {
	...
	if(queueId == actuator->queue_id) {
		if(init){
			IMPL_AX12_POS_DOUBLE_command_position_init(actuator);
		} else {
			IMPL_AX12_POS_DOUBLE_command_position_run(actuator);
		}
	}
}
...
static void IMPL_AX12_POS_DOUBLE_command_position_init(Actuator_servo_double_t* actuator) {
	...
	if(order == ACT_DEFAULT_STOP) {
		if(!global.flags.virtual_mode) {
			for(i = 0; i < 2; i++) {
				AX12_set_torque_enabled(actuator->servo[i].id, FALSE); //Stopper l'asservissement de l'AX12
			}
		}
...
//	}
	...
	for(i = 0; i < 2; i++) {
		if(global.flags.virtual_mode) {
			...
			AX12_reset_last_error(actuator->servo[i].id); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
			AX12_set_torque_enabled(actuator->servo[i].id, TRUE); // Activation de l'asservissement de l'AX12
			if(!AX12_set_position(actuator->servo[i].id, goal_position[i])) {
...
static void IMPL_AX12_POS_DOUBLE_command_position_run(Actuator_servo_double_t* actuator) {
	...
	for(i = 0; i < 2; i++) {
		if(!check_finish[i]) {
			...
			if(global.flags.virtual_mode) {
				...
				pos[i] = AX12_get_position(actuator->servo[i].id);
				check_finish[i] = ACTQ_check_status_ax12(actuator->queue_id, actuator->servo[i].id, wanted_goal[i], pos[i], actuator->servo[i].epsilon, actuator->servo[i].timeout, actuator->servo[i].large_epsilon, &result[i], &error_code[i], &line[i]);
			}
		}
	}
	...
	if(actuator->warner.activated)
		IMPL_AX12_POS_DOUBLE_check_warner(actuator, pos[0]);
}
...
static void IMPL_AX12_POS_DOUBLE_add_torque_order(Actuator_servo_double_t *actuator, Uint8 cmd_index) {
	...
	if(threshold[0] <= 100 && threshold[1] <= 100) {
		QUEUE_add(actuator->queue_id, &IMPL_AX12_POS_DOUBLE_run_command_torque, (QUEUE_arg_t){ACT_SERVO_ASSER_TORQUE, cmd_index, &ACTQ_finish_SendNothing, TRUE});
	} else {
 ...
 * @param init TRUE au premier appel pour initiliser la commande et FALSE ensuite.
 */
void IMPL_AX12_POS_DOUBLE_run_command_torque(queue_id_t queueId, void* actuator_gen_ptr, bool_e init) {
	Actuator_servo_double_t* actuator = (Actuator_servo_double_t*) actuator_gen_ptr;
...
void IMPL_AX12_POS_DOUBLE_set_warner(void* actuator_gen_ptr, CAN_msg_t *msg) {
	...
	if(warner_is_valid){
		...
		actuator->warner.last_value = AX12_get_position(actuator->servo[0].id);
	} else {
	...
	if(actuator->servo[0].torque_asser.activated && actuator->servo[1].torque_asser.activated) {
		if(actuator->servo[0].torque_asser.ask_to_finish || actuator->servo[1].torque_asser.ask_to_finish ) {
			...
			for(i = 0; i < 2; i++) {
				...
				current_load[i] = - AX12_get_load_percentage(actuator->servo[i].id);
				current_pos[i] = AX12_get_position(actuator->servo[i].id);
			...
			for(i = 0; i < 2; i++) {
				...
				if(!AX12_set_position(actuator->servo[i].id, new_pos[i])) {
