 ...
 */
	...
	#define SIMU_FILTER(sid)			((sid >= 0x760) && (sid <= 0x7FF))
	...
	typedef union{
		...
		#define XBEE_ELEMENTS_COLOR_COMBINATION			0x5B7
		#define SIZE_XBEE_ELEMENTS_COLOR_COMBINATION	7
			struct{
				CUBE_color_e first_color		:8;
				CUBE_color_e second_color		:8;
				CUBE_color_e third_color		:8;
				time32_t match_time				:32;	// On envoie aussi le temps de match pour le panneau
			}xbee_elements_color_combination;

		#define XBEE_SCORE_ESTIMATION			0x5B8
		#define SIZE_XBEE_SCORE_ESTIMATION		8
			struct{
				Uint16 estimation				:16;
				Uint8 foe_ball_placed			:4;
				Uint8 our_ball_placed			:4;
				Uint8 water_getted				:4;
				Uint8 first_stage				:4;
				Uint8 second_stage				:4;
				Uint8 third_stage				:4;
				Uint8 fourth_stage				:4;
				Uint8 fifth_stage				:4;
				Uint8 well_done_building		:4;
				bool_e bee_placed				:1;
				bool_e bursted_balloon			:1;
				bool_e domotic_placed			:1;
				bool_e switch_activated			:1;
				robot_id_e robot_id				:2;
			}xbee_score_estimation;
		 ...
		 **********************************************************************************************************************
		 *********************************************************************************************************************/
		#define BROADCAST_SELFTEST_BEGIN	0x0DB	//DéBut du selftest : on prévient les autres cartes qu'un selftest commence !
		#define BROADCAST_SELFTEST_END		0x0F1	//Fin du Selftest : on prévient les autres cartes que le seltest est terminé.
	 ...
	 *********************************************************************************************************************/
		...
		#define ACT_SET_CONFIG						0x3A1
		#define SIZE_ACT_SET_CONFIG					8
			struct{
				Uint8 act_sid						:8;
				act_config_e config					:8;
				union{
		...
		#define ACT_WARNER							0x3A2
		#define SIZE_ACT_WARNER						2
			struct{
				Uint8 act_sid						:8;
				Uint8 warner_param					:8;
			}act_warner;
		...
		#define ACT_EXPANDER_SET_PUMP						0x3E0
		#define SIZE_ACT_EXPANDER_SET_PUMP					3
			struct{
				actExpanderId_e id							:8;
				Uint8 idPump								:8;		// 0 - 5
				bool_e state								:8;
			}actExpander_setPump;

		#define ACT_EXPANDER_RESULT_SET_PUMP				0x224
		#define SIZE_ACT_EXPANDER_RESULT_SET_PUMP			3
			struct{
				actExpanderId_e id							:8;
				Uint8 idPump								:8;		// 0 - 5
				bool_e state								:8;
			}actExpander_resultSetPump;

		#define ACT_EXPANDER_SET_SOLENOID_VALVE				0x3DF
		#define SIZE_ACT_EXPANDER_SET_SOLENOID_VALVE		5
			struct{
				actExpanderId_e id							:8;
				Uint8 idSolenoidValve						:8;		// 0 - 5
				time32_t duration							:16;	// ms
				bool_e state								:8;
			}actExpander_setSolenoideValve;

		#define ACT_EXPANDER_RESULT_SET_SOLENOID_VALVE		0x225
		#define SIZE_ACT_EXPANDER_RESULT_SET_SOLENOID_VALVE	3
			struct{
				actExpanderId_e id							:8;
				Uint8 idSolenoidValve						:8;		// 0 - 5
				bool_e state								:8;
			}actExpander_resultSetSolenoideValve;

		#define ACT_EXPANDER_GET_PUMP_STATUS				0x3DE
		#define SIZE_ACT_EXPANDER_GET_PUMP_STATUS			2
			struct{
				actExpanderId_e id							:8;
				Uint8 idPump								:8;		// 0 - 5
			}actExpander_getPumpStatus;

		#define ACT_EXPANDER_TELL_PUMP_STATUS				0x250
		#define SIZE_ACT_EXPANDER_TELL_PUMP_STATUS			3
			struct{
				actExpanderId_e id							:8;
				Uint8 idPump								:8;		// 0 - 5
				ACT_EXPANDER_pumpStatus_e status			:8;
			}actExpander_tellPumpStatus;

		#define ACT_EXPANDER_GET_COLOR_SENSOR				0x3DD
		#define SIZE_ACT_EXPANDER_GET_COLOR_SENSOR			2
			struct{
				actExpanderId_e id							:8;
				Uint8 idSensor								:8;		// 0 - 5 ACT_EXPANDER_pumpStatus_e
			}actExpander_getColorSensor;

		#define ACT_EXPANDER_TELL_COLOR_SENSOR				0x227
		#define SIZE_ACT_EXPANDER_TELL_COLOR_SENSOR			3
			struct{
				actExpanderId_e id							:8;
				Uint8 idSensor								:8;		// 0 - 5
				ACT_EXPANDER_colorSensor_e sensorColor		:8;
			}actExpander_tellColorSensor;

		#define ACT_EXPANDER_GET_DISTANCE_SENSOR			0x3DC
		#define SIZE_ACT_EXPANDER_GET_DISTANCE_SENSOR		2
			struct{
				actExpanderId_e id							:8;
				Uint8 idSensor								:8;		// 0 - 5
			}actExpander_getDistanceSensor;

		#define ACT_EXPANDER_SET_DISTANCE_SENSOR_THRESHOLD	0x3DB
		#define SIZE_ACT_EXPANDER_SET_DISTANCE_SENSOR_THRESHOLD		5
			struct{
				actExpanderId_e id							:8;
				Uint8 idSensor								:8;		// 0 - 5
				Uint16 threshold							:16;	// [mm]
				ACT_EXPANDER_DistanceThresholdWay_e way		:8;
			}actExpander_setDistanceSensorThreshold;

		#define ACT_EXPANDER_TELL_DISTANCE_SENSOR			0x228
		#define SIZE_ACT_EXPANDER_TELL_DISTANCE_SENSOR		4
			struct{
				actExpanderId_e id							:8;
				Uint8 idSensor								:8;		// 0 - 5
				Uint16 distance								:16;	// [mm]
				bool_e threshold							:8;
			}actExpander_tellDistanceSensor;


		/**********************************************************************************************************************
		 **********************************************************************************************************************
		 **										Carte Strat <-> Carte Mosfet												 **
		 **********************************************************************************************************************
		 *********************************************************************************************************************/
		...
		#define SIZE_MOSFET_BOARD_TELL_TURBINE_SPEED		2
			struct{
				Uint16 speed								:16;		// [rpm]
			}mosfet_board_tell_turbine_speed;

		#define MOSFET_BOARD_SET_TURBINE_SPEED				0x505
		#define SIZE_MOSFET_BOARD_SET_TURBINE_SPEED			2
			struct{
				Uint16 speed								:16;		// [rpm]
	 ...
	 *********************************************************************************************************************/
		...
		#define SIMU_ASK_WHO_I_AM			0x760
		#define SIZE_SIMU_ASK_WHO_I_AM		0

		#define SIMU_WHO_I_AM				0x761
		#define SIZE_SIMU_WHO_I_AM			2
			struct{
				robot_id_e robot_id			:8;
				board_id_e board_id			:8;
			}simu_who_i_am;

		#define SIMU_ASK_GET_SWITCH			0x762
		#define SIZE_SIMU_ASK_GET_SWITCH	0

		#define SIMU_GET_SWITCH				0x763
		#define SIZE_SIMU_GET_SWITCH		4
			struct{
				Uint32 switch_mask			:32;
			}simu_get_switch;

		#define SIMU_PING					0x764
		#define SIZE_SIMU_PING				0

		#define SIMU_PONG					0x765
		#define SIZE_SIMU_PONG				0
	...
	typedef enum {
		...
		//EXEMPLE								= (ACT_FILTER | 0x01),
		ACT_BIG_CROSS_TAKER_FRONT				= (ACT_FILTER | 0x01),
		ACT_BIG_CROSS_TAKER_BACK				= (ACT_FILTER | 0x02),
		ACT_BIG_ELEVATOR						= (ACT_FILTER | 0x03),
		ACT_BIG_ELEVATOR_ARM_VERY_BACK			= (ACT_FILTER | 0x04),
		ACT_BIG_ELEVATOR_ARM_BACK				= (ACT_FILTER | 0x05),
		ACT_BIG_ELEVATOR_ARM_FRONT				= (ACT_FILTER | 0x06),
		ACT_BIG_ELEVATOR_ARM_VERY_FRONT			= (ACT_FILTER | 0x07),
		ACT_BIG_PACKIX							= (ACT_FILTER | 0x08),
		ACT_BIG_BALL_TAKER_FRONT				= (ACT_FILTER | 0x09),
		ACT_BIG_BALL_TAKER_BACK					= (ACT_FILTER | 0x0A),
		ACT_BIG_BALL_LAUNCHER					= (ACT_FILTER | 0x0B),
		ACT_BIG_BEE_PUSHER_ARM					= (ACT_FILTER | 0x0C),
		ACT_BIG_BEE_PUSHER_TURRET				= (ACT_FILTER | 0x0D),
		ACT_BIG_SWITCH_PUSHER					= (ACT_FILTER | 0x0E),

		//////////////////////////////////////////////////////////////////
		//--------------------------SMALL_ROBOT-------------------------//
		//////////////////////////////////////////////////////////////////

		//EXEMPLE						= (ACT_FILTER | 0x31),
		ACT_SMALL_SWITCH_PUSHER			= (ACT_FILTER | 0x31),
		ACT_SMALL_BALL_TAKER			= (ACT_FILTER | 0x32),
		ACT_SMALL_CROSS_TAKER			= (ACT_FILTER | 0x33),
		ACT_SMALL_ELEVATOR_ARM_BACK		= (ACT_FILTER | 0x34),
		ACT_SMALL_ELEVATOR_ARM_FRONT	= (ACT_FILTER | 0x35),
		ACT_SMALL_PACKIX				= (ACT_FILTER | 0x36),
		ACT_SMALL_GOLDEN_FITTER			= (ACT_FILTER | 0x37),
		ACT_SMALL_ELEVATOR				= (ACT_FILTER | 0x38),
		ACT_SMALL_BEE_PUSHER_ARM		= (ACT_FILTER | 0x39),
		ACT_SMALL_BALL_PIPE_ROTATOR		= (ACT_FILTER | 0x40),
		ACT_SMALL_BALL_ADV_LIBERATOR	= (ACT_FILTER | 0x41),
		ACT_SMALL_BALL_OUR_LIBERATOR	= (ACT_FILTER | 0x42),
